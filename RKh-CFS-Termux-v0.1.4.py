...
def latency_sort_key(r: ScanResult) -> Tuple[float, str]:
    """Initial scan output: sort only by latency, low to high."""
    latency = r.latency_ms if r.latency_ms is not None else 10**12
    return (latency, r.ip)


def recheck_sort_key(r: ScanResult) -> Tuple[float, int, str]:
    """Re-check output: average latency first, then more passed samples."""
    latency = r.latency_ms if r.latency_ms is not None else 10**12
    return (latency, -r.recheck_passed, r.ip)


def speed_sort_key(r: ScanResult) -> Tuple[float, str]:
    """Speed-test output: sort only by download speed, high to low."""
    speed = r.speed_mbps if r.speed_mbps is not None else -1.0
    return (-speed, r.ip)


def result_sort_key(r: ScanResult) -> Tuple[float, str]:
    """Default/live scan ordering: latency only."""
    return latency_sort_key(r)


def output_sort_key(filename_prefix: str):
...
def run_speed_tests(v: VlessConfig, ok_results: List[ScanResult], xray: Path, workers: int, timeout: int, speed_bytes: int, speed_duration: int, speed_url_template: str, loglevel: str) -> List[ScanResult]:
    ...
    if RICH:
        ...
        title = f"Speed testing: {mb:.1f} MB download per IP{duration_text}"
        ...
        with progress:
            ...
            futures = {}
            try:
                futures = {ex.submit(speed_test_ip, v, r, xray, timeout, speed_bytes, speed_duration, speed_url_template, loglevel): r.ip for r in targets}
                for fut in as_completed(futures):
                    r = fut.result()
                    results.append(r)
                    if r.speed_mbps is not None:
                        console.print(f"[bright_green]SPD[/bright_green] {r.ip:<39} [bold]{r.speed_mbps:.2f} Mbps[/bold]")
                    else:
        ...
        try:
            ...
            for fut in as_completed(futures):
                ...
                if r.speed_mbps is not None:
                    print(f"SPD {r.ip:<39} {r.speed_mbps:.2f} Mbps")
...
def save_results(results: List[ScanResult], output_dir: Path, filename_prefix: str = "clean_ips", max_latency_ms: float = 0, min_speed_mbps: float = 0, include_speed_errors: bool = False) -> Tuple[Path, Path]:
    """Save results with independent ordering per stage.

    - clean_ips.*: sorted by latency only.
    - clean_ips_rechecked.*: sorted by average latency, then passed sample count.
    - clean_ips_speed_tested.*: sorted only by download speed.
    """
    ...
    is_speed = "speed" in filename_prefix
    is_recheck = "rechecked" in filename_prefix
    ...
    if not ok:
        ...
        for r in ok:
            ...
            if is_speed:
                lines.append(f"{r.ip} | speed: {speed} | downloaded: {r.speed_bytes} bytes{extra}")
            elif is_recheck:
                lines.append(f"{r.ip} | latency_avg: {latency} | recheck_passed: {r.recheck_passed}/{r.recheck_total} | HTTP {r.status_code}")
            else:
                lines.append(f"{r.ip} | latency: {latency} | HTTP {r.status_code}")
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    with csv_path.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        if is_speed:
            w.writerow(["ip", "speed_mbps", "speed_bytes", "speed_error", "tested_at"])
            for r in ok:
                w.writerow([
                    r.ip,
            ...
            w.writerow(["ip", "avg_latency_ms", "recheck_passed", "recheck_total", "status_code", "tested_at"])
            for r in ok:
                w.writerow([
                    r.ip,
...
def summarize_latency(samples: List[ScanResult]) -> ScanResult:
    ...
    if not ok_samples:
        ...
        return ScanResult(ip=ip, ok=False, error=err, recheck_passed=0, recheck_total=total)
    # Use average latency as the final score, while keeping the number of passed real Xray samples for ranking/output.
    avg = sum(r.latency_ms or 0 for r in ok_samples) / passed
    status = ok_samples[-1].status_code
    return ScanResult(ip=ip, ok=True, latency_ms=avg, status_code=status, error=f"{passed}/{total} successful samples", recheck_passed=passed, recheck_total=total)
    ...
    if RICH:
        ...
        with progress:
            ...
            try:
                ...
                for fut in as_completed(futures):
                    ...
                    if r.ok:
                        console.print(f"[bright_green]RE-OK[/bright_green] {r.ip:<39} [bold]{r.latency_ms:.1f} ms avg[/bold]  pass {r.recheck_passed}/{r.recheck_total}")
                    else:
        ...
        try:
            ...
            for fut in as_completed(futures):
                ...
                results.sort(key=recheck_sort_key)
                if r.ok:
                    print(f"RE-OK {r.ip:<39} {r.latency_ms:.1f} ms avg pass {r.recheck_passed}/{r.recheck_total}")
            ...
            while True:
                ...
                if do_recheck:
                    ...
                    if sample_count and recheck_workers:
                        ...
                        try:
                            ...
                            show_final_results(final_results, retxt, recsv, filename_prefix="clean_ips_rechecked")
                            return
                        show_final_results(final_results, retxt, recsv, filename_prefix="clean_ips_rechecked")
                ...
                if do_speed:
                    try:
                        ...
                        speed_duration = prompt_int_nav("Speed test duration per IP seconds", 5, 1, 120, allow_back=True)
                        speed_workers = prompt_int_nav("Speed test workers", min(concurrency, 5), 1, 50, allow_back=True)
                        speed_timeout = prompt_int_nav("Speed test timeout seconds", max(timeout, speed_duration + 10), 5, 180, allow_back=True)
                        ...
                        do_speed = False
                    if do_speed:
                        render_stage("Speed Testing", f"Download size per IP: {speed_mb} MB | Duration/IP: {speed_duration}s | Workers: {speed_workers}", "magenta")
                        try:
                            final_results = run_speed_tests(v, final_results, xray, speed_workers, speed_timeout, speed_mb * 1024 * 1024, speed_duration, speed_url, "warning")
                            ...
                            show_final_results(final_results, stxt, scsv, filename_prefix="clean_ips_speed_tested")
                            return
                        show_final_results(final_results, stxt, scsv, filename_prefix="clean_ips_speed_tested")
                        if min_speed > 0 or max_latency > 0:
                            ...
                            show_final_results(final_results, ftxt, fcsv, max_latency_ms=max_latency, min_speed_mbps=min_speed, filename_prefix="clean_ips_speed_filtered")
...
def show_final_results(results: List[ScanResult], txt_path: Path, csv_path: Path, max_latency_ms: float = 0, min_speed_mbps: float = 0, filename_prefix: str = "clean_ips") -> None:
    ok = apply_result_filters(results, max_latency_ms=max_latency_ms, min_speed_mbps=min_speed_mbps)
    ok = sorted(ok, key=output_sort_key(filename_prefix))
    ...
    if RICH:
        ...
        if is_speed:
            ...
            for i, r in enumerate(ok[:25], 1):
                speed = f"{r.speed_mbps:.2f} Mbps" if r.speed_mbps is not None else "-"
                table.add_row(str(i), r.ip, speed, str(r.speed_bytes or "-"))
        elif is_recheck:
            table.add_column("Latency Avg", justify="right", style="green")
            table.add_column("Passed", justify="right", style="magenta")
            table.add_column("HTTP", justify="right")
            for i, r in enumerate(ok[:25], 1):
                latency = f"{r.latency_ms:.1f} ms" if r.latency_ms is not None else "-"
                recheck = f"{r.recheck_passed}/{r.recheck_total}" if r.recheck_total else "-"
                table.add_row(str(i), r.ip, latency, recheck, str(r.status_code))
        else:
            table.add_column("Latency", justify="right", style="green")
            table.add_column("HTTP", justify="right")
            for i, r in enumerate(ok[:25], 1):
                latency = f"{r.latency_ms:.1f} ms" if r.latency_ms is not None else "-"
                table.add_row(str(i), r.ip, latency, str(r.status_code))
        console.print(table)
        ...
        for i, r in enumerate(ok[:25], 1):
            if is_speed:
                speed = f"{r.speed_mbps:.2f} Mbps" if r.speed_mbps is not None else "-"
                print(f"{i}. {r.ip} speed={speed} downloaded={r.speed_bytes or '-'}")
            elif is_recheck:
                latency = f"{r.latency_ms:.1f} ms" if r.latency_ms is not None else "-"
                recheck = f"{r.recheck_passed}/{r.recheck_total}" if r.recheck_total else "-"
                print(f"{i}. {r.ip} latency_avg={latency} pass={recheck} HTTP {r.status_code}")
            else:
                latency = f"{r.latency_ms:.1f} ms" if r.latency_ms is not None else "-"
                print(f"{i}. {r.ip} latency={latency} HTTP {r.status_code}")
...
def main() -> int:
    ...
    if args.recheck:
        try:
            ...
            show_final_results(final_results, txt, csvp, filename_prefix="clean_ips_rechecked")
            return 130
        show_final_results(final_results, txt, csvp, filename_prefix="clean_ips_rechecked")
    if args.speed_test:
        try:
            final_results = run_speed_tests(v, final_results, xray, args.speed_workers, args.speed_timeout, args.speed_mb * 1024 * 1024, args.speed_duration, args.speed_url, args.loglevel)
            ...
            show_final_results(final_results, txt, csvp, filename_prefix="clean_ips_speed_tested")
            return 130
        show_final_results(final_results, txt, csvp, filename_prefix="clean_ips_speed_tested")
        if args.max_latency > 0 or args.min_speed > 0:
            ...
            show_final_results(final_results, txt, csvp, max_latency_ms=args.max_latency, min_speed_mbps=args.min_speed, filename_prefix="clean_ips_speed_filtered")
    return 0
